...
```
.
├── main.go                 # 入口：配置 → ui.RunShell → ScriptController → buildRuntime
...
3. 用户点运行 → `ScriptController.Start` → `ApplyToConfig` → `buildRuntime(cfg, statusReporter)`
4. `buildRuntime` 内：
...
`main.go` 已固定装配顺序，新任务只需扩展 binding 与面板，不必改生命周期：
...
```go
seedBool(store, KeyCollectEnabled, cfg.Tasks.Collect.Enabled)
```

3. `ApplyToConfig`：从 Store 写回 `cfg`（运行前必须能读到最新值）：

```go
cfg.Tasks.Collect.Enabled = store.GetBool(KeyCollectEnabled)
```
...
面板是「分类 + 列表 + 详情」：任务写在编译期静态表 [`BuiltinTasks`](../internal/ui/tasks.go)，**不要**再引入全局 `RegisterModule` 注册表。新任务两步：

1. 在 `BuiltinTasks()` 追加一项（`ID` / `Title` / `Category` / `EnabledKey`）。
2. 在 `cookie_panel_android.go` 的 `renderTaskDetail` switch 里加详情控件（key 与 binding 一致）。
...
- 桌面 `panel_stub` 会跳过面板直接走 `ScriptController.Start()`，不渲染这些控件。
- 未实现的任务不要进 `BuiltinTasks` / 面板。

#### 6.4.3 UI 改完后的自检

- [ ] `binding` 常量、`SeedFromConfig`、`ApplyToConfig`、面板控件四处 key 一致
- [ ] `CheckEnabled`（或任务逻辑）读的是 `cfg.Tasks.Xxx`，且发生在 `ApplyToConfig` 之后（`ScriptController.OnStart` 已保证）
...
- **任务状态上岛**：任务把一行状态文本写进 `internal/status.Reporter`（任务 goroutine 写、UI 每帧读，线程安全；空串回退默认文案）。竞技场在状态机 `sync` 节点调用 `Task.pushStatus`（文案由 `State.StatusText` 生成：`竞技场 3/10 · 胜率 50%`）；`main.go` 创建唯一实例，同时传给 `ShellOptions.Status` 与 `Task.SetStatusReporter`。仅 **运行中** 展示任务状态，暂停/空闲回退 `帅宾 Cookie · 状态`。新任务要上报：调 `SetStatusReporter` 并在循环节点推文本即可。
- Android：`internal/ui` ImGui 面板，配置落盘路径默认 `ui.DefaultConfigPath`
- 非 Android：`panel_stub.go` 直接 `Controller.Start()`（无面板渲染），方便本地联调装配

新增任务时的完整 UI 步骤见 **§6.4**（binding key → Seed/Apply → `cookie_panel_android.go` 控件 → 自检）。通用控件在 `widgets_android.go`，优先复用，勿复制一套。
