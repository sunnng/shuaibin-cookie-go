...
2. `ui.LogErrorf = logger.Errorf`；`ui.NewStore`；`status.New()` 创建任务状态上报；组合 `[]ui.Task`（`taskdesc.Tasks(cfg)`，描述符在 `internal/taskdesc`）与 `ui.ScriptController`；`ui.RunShell(ShellOptions{Tasks, Nav, Controller, Status: statusReporter, ConfigPath, DataStorePath, ...})`（灵动岛 + 面板；首帧 LoadConfig 后由 `shell.Seed()` 填充缺失键，见 §6.4）
...
文件：应用侧描述符包 [`internal/taskdesc/`](../internal/taskdesc/)（每任务一个文件，如 `arena.go`），`main.go` 只调 `taskdesc.Tasks(cfg)` 聚合，**不再**改旧的 binding 文件。
...
| `int` | `ui.Number` | 面板渲染为步进数字输入框（get/set 为 int；float 字段需在闭包内换算） |
...
- 悬浮窗为**灵动岛**（[`ui/island_android.go`](../ui/island_android.go)）：顶部居中深色小圆，无文字，仅中央状态点 + 3px 状态色描边表达状态（空闲蓝 / 运行绿 / 暂停橙，糖果积木 §4.1 胶囊去文案的极简形态）；点按展开深色圆角卡片（开始/停止、暂停/继续、设置、关闭四个图标按钮，卡片顶部状态行显示任务状态文本），再点卡片空白处或卡片外任意处收起，展开约 6s 无操作自动收起；胶囊按住可拖动，位置持久化到 ui.json（`island_pos_x`/`island_pos_y`）。回调直连 `ui.Shell` 方法（`shell.StartStop` / `PauseResume` / `OpenPanel` / `Exit`），不再有 `IslandCallbacks`。
