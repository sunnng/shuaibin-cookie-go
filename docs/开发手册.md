...
1. `guard.Check()` — 处理已注册弹窗陷阱（当前生产可零注册；框架已接线，陷阱待按弹窗取色后 `Register`）
...
│   ├── guard/              # 弹窗陷阱 + 分段 sleep
│   ├── logger/             # 分级日志
│   ├── store/              # 业务 JSON KV（如免费刷新时间；设备路径 /sdcard/shuaibin-cookie/store.json）
│   ├── platform/
...
2. `ui.NewStore` + `SeedFromConfig` → `ui.RunShell`（悬浮球 + 面板）
3. 用户点运行 → `SessionController.Start` → `ApplyToConfig` → `buildRuntime(cfg)`
4. `buildRuntime` 内：
  - `screen.NewDetector` / `action.NewExecutor`
  - `store.New("/sdcard/shuaibin-cookie/store.json")` / `guard.New` / `scheduler.New`
  - 构造模块 Page / Session / Task
  - `sched.Build(TaskOpts{...})`**（可多次）**
  - `runtime.New(...)`，`arenaTask.SetShouldStop(rt.IsStopped)`，返回 `rt`
5. Controller 在 goroutine 中调用 `rt.Run()`

不要使用已删除的 `runtime.Register`；任务必须在 `Run()` 之前注册完毕。`RunPanel` 仅为兼容入口，新代码只用 `RunShell`。
...
3. 用户在面板改控件 → 写回 `Store`
4. 点「运行脚本」→ `ui.ApplyToConfig(store, cfg)` → `buildRuntime(cfg)` → `rt.Run()`
...
- 用户选项落盘路径默认：`/sdcard/shuaibin-cookie/ui.json`（`ShellOptions.ConfigPath`）。这是 UI Store 存档，与项目根目录 `config.json` 不同：前者偏设备上的面板状态，后者偏仓库默认/文档化配置。业务 KV（如竞技场刷新时间）另存 `/sdcard/shuaibin-cookie/store.json`。
- 首页「说明」文案如需提到新模块，可改 `renderHomeTab`，非必须。
- 桌面 `panel_stub` 会跳过面板直接走 Controller，不渲染这些控件。
