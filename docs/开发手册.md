...
2. `ui.LogErrorf = logger.Errorf`；`ui.NewStore`；`status.New()` 创建任务状态上报；组合 `[]ui.Task`（如 `arenaTaskDescriptor(cfg)`）与 `ui.ScriptController`；`ui.RunShell(ShellOptions{Tasks, Nav, Controller, Status: statusReporter, ConfigPath, DataStorePath, ...})`（灵动岛 + 面板；首帧 LoadConfig 后由 `shell.Seed()` 填充缺失键，见 §6.4）
