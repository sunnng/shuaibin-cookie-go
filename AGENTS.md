...
3. Wire the UI: add a `ui.Task` descriptor in the `main.go` composition root (use `ui.Bool` / `ui.Number` / `ui.Text` for fields; the key is the device `ui.json` persistence key). Details render via the default Form automatically; use `RenderDetail` only when custom layout is needed.
