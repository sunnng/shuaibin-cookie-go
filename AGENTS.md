...
```
.
├── main.go                      # Entry: config → ui.RunShell (floating ball + panel) → SessionController builds runtime on Start
...
│   ├── store/                   # JSON file-backed key-value store (data/store.json)
│   └── ui/                      # ImGui shell: floating ball, config panel, SessionController, config binding (android&&cgo) / stub (desktop)
...
- **Runtime loop**: `internal/runtime/runtime.go` runs a forever loop of `guard.Check → scheduler.Run → idle wait`. Idle sleep uses `scheduler.MaxIdleWait()` (longest `CheckReady` remain across tasks) capped by `IdleDelay`; after work it sleeps `StepDelay`. Supports `Pause`/`Resume`/`Stop`, driven by the UI controller.
...
- **UI shell & config binding**: on Android, `ui.RunShell` runs the ImGui loop (floating ball always drawn + flat config panel). `SessionController` (Idle/Running/Paused) starts the bot in a goroutine and rebuilds the runtime on every Start. Config flows one way: `config.json` → `SeedFromConfig` (fills only keys missing from the device store at `/sdcard/shuaibin-cookie/ui.json`) → panel widgets mutate the `ui.Store` → on Start, `ApplyToConfig` writes the store back into `config.Config` → `buildRuntime(cfg)`. Adding a user setting means four edits that must use the **same key**: a `KeyXxx` const in `internal/ui/binding.go`, `SeedFromConfig`, `ApplyToConfig`, and the widget in `cookie_panel_android.go` — otherwise panel changes silently don't take effect. Panel is list+detail with categories over a compile-time `BuiltinModules()` table — do **not** reintroduce a global `RegisterModule` registry. See `docs/开发手册.md` §6.4.
...
- **README.md is stale** — it describes the removed `internal/bot` state architecture (Detect/Act/Next/Recover). Trust this file, `CLAUDE.md`, and `docs/开发手册.md` instead.
...
4. Register with `sched.Build(scheduler.TaskOpts{...})` in `main.go`'s `buildRuntime`, before `rt.Run()`. Scheduling order = `Build` call order; put urgent tasks first, fallbacks last.
