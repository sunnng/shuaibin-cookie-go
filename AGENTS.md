...
- Stub factories carry `//go:build !android || !cgo`: `internal/platform/screen/factory.go`, `internal/platform/action/factory.go`, and `ui/run_stub.go` (plus page stubs) provide no-op implementations so the project type-checks and tests run anywhere.

## Project Structure

```
.
├── main.go                      # Entry: config → ui.RunShell (descriptors: Tasks/Nav/Controller); OnStart ApplyAll → buildRuntime
...
│   ├── game/                    # Game tasks (arena; mine survey/mining/battle/jelly; square; starlight; market; biscuit; common/kingdom + common/popup shared)
...
│   ├── status/                  # One-line task-status reporter: task goroutine writes, expanded island card reads
│   ├── store/                   # JSON file-backed key-value store (device path: main.go defaultStorePath)
│   └── taskdesc/                # App-side task panel descriptors (one ui.Task per game task) + Tasks(cfg) aggregate
...
- **Task-level state machines**: each game task (e.g. `internal/game/arena`) owns an `internal/statemachine.Machine` for its internal flow (`detect → navigate → sync → check → selectOpponent → teamSelect → battle → leave`). Handlers return `Keep` / `Retry` (bounded by MaxRetry) / `Next("state")` / `Done` / `Fatal{Err}`; a task can hook `RunOptions.Guard = guard.Check` so popups interrupt its ticks. Do not reintroduce a global bot state machine — page flow lives in each task.
- **UI shell & config binding**: on Android, `ui.RunShell(ShellOptions)` runs the ImGui main loop (Dynamic-Island capsule always drawn + Candy-Blocks themed config panel — 糖果积木 neo-brutalism: paper bg, 3px ink borders, hard shadows, candy accent colors per `docs/ui-redesign/design-system.md`; theme tokens are `ui.DefaultTheme()` and `ui/theme_apply_android.go`, shared draw helpers live in `ui/candy_android.go`). `ScriptController` (Idle/Running/Paused) starts the bot in a goroutine and rebuilds the runtime on every Start. The wiring is descriptor-based: the main.go composition root builds a `[]ui.Task` via `taskdesc.Tasks(cfg)` (descriptors live in `internal/taskdesc`, one file per task) (each `ui.Task` carries `ID`, `Title`, `Category`, `EnabledKey`, and `Fields`), and mounts `ui.TaskListPage()` and `ui.SystemPage()` as `Nav` entries — there is no global `RegisterModule` registry. Config flow: `config.json` → `config.LoadConfig` → `shell.Seed()` (fills only keys missing from the device store at `/sdcard/shuaibin-cookie/ui.json`, so the device store wins) → panel widgets mutate the `ui.Store` → on Start, the shell saves the store to `ConfigPath` and the hook calls `ui.ApplyAll` to write the store back into `config.Config` → `buildRuntime(cfg, statusReporter)`. Adding a user setting means a single edit: declare one `ui.Bool`, `ui.Number`, or `ui.Text` field in the task descriptor with the same key used for persistence in `/sdcard/shuaibin-cookie/ui.json`. The overlay is captured by `CaptureScreen` along with the game and cannot be excluded from it, so the policy is "don't recognize while occluded": opening the panel auto-pauses a running script and closing it auto-resumes (`autoPaused` in `ui.Shell`), the expanded island card auto-collapses after ~6s idle, and the island pill is draggable (press-hold to move; position persisted in ui.json as `island_pos_x`/`island_pos_y`) so users can drag it off key game data — it defaults to top-center, so recognition regions in feature files must still avoid the default strip (1600-base y≤60; expanded card y≤180, x≈575–1025). Task status reporting: tasks push a one-line status (arena: battles/win-rate, from `State.StatusText`) through `internal/status.Reporter`, which implements the framework's `ui.StatusSource` interface; `main.go` shares one instance between `ShellOptions.Status` and `Task.SetStatusReporter`, and the expanded island card status row shows it in place of the default label while Running; the collapsed pill is a small ink-dark circle whose centered state dot and border color alone signal state (idle blue / running green / paused orange). See `docs/开发手册.md` §6.4.
- **Interfaces for testability**: `platform/screen.Detector` and `platform/action.Executor` are interfaces. Tasks define narrow consumer-side interfaces (arena's `page` / `route` in `task.go`) so tests inject mocks via unexported constructors (`newTask`) and drive machines with short intervals (`runWithOptions`). Unit tests mock them so the scheduler, runtime, guard, and state machines can be tested without Android runtime.
...
- **Config**: `internal/config/static.go` defines defaults and `internal/config/user.go` loads `config.json`. Missing `config.json` falls back to `DefaultConfig()`. Stable UI constants belong in task code, not JSON.
...
- **Dev manual (authoritative, Chinese)**: `docs/开发手册.md` — detailed conventions: task creation, UI binding, platform layer, testing, pitfalls (§13), pre-PR checklist (§15).
- Full AutoGo API reference: `docs/autogo-doc文档2026.6.6.md`.
- Design specs: `docs/superpowers/specs/` (arena task, feature layout, recognition, floating ball, architecture simplify, ...).
...
1. Create a package under `internal/game/`, following the convention: `feature.go` (UI constants only — regions/points/color strings at 1600×900 base; organized per page with `Identify`/`Actions`/`Reads` slots, see `internal/game/arena/feature.go`), `page.go`, `route.go`, `state.go`, `task.go`, `statemachine.go`. Shared navigation goes under `internal/game/common/`.
2. Add config fields in `internal/config/static.go` (+ `DefaultConfig`, `config_test.go`). **Do not add config/UI for unimplemented tasks.**
3. Wire the UI: add a `ui.Task` descriptor in `internal/taskdesc` (one file per task; include it in `Tasks()`) (use `ui.Bool` / `ui.Number` / `ui.Text` for fields; the key is the device `ui.json` persistence key). Details render via the default Form automatically; use `RenderDetail` only when custom layout is needed.
...
- Task names and many log messages are Chinese (e.g. `"王国竞技场"`); keep the `[Task]` log prefix convention (`[Runtime]`, `[Arena]`, `[Guard]`).
- `.worktrees/` holds git worktrees for feature branches and is gitignored.
- `CLAUDE.md` mirrors this file for Claude Code — keep the two in sync when editing either.
